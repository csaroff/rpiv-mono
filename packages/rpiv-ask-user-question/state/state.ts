...
export interface QuestionnaireState {
	...
	notesDraft: string;
	/**
	 * Collapsed mode: the questionnaire shrinks to a single hint row so the agent
	 * transcript above it is easier to read. Toggled by the configured collapse key
	 * from any state; while true, every keystroke except cancel is swallowed (see
	 * `routeKey`). The dialog renders inline in the editor slot, so the row keeps
	 * focus and input routing and the same key expands it again.
	 */
	collapsed: boolean;
}
