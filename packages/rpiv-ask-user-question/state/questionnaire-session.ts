...
export class QuestionnaireSession {
	...
	/**
	 * Collapsed render: a single dim row in the editor slot. Returning one line
	 * shrinks the inline dialog from its full height to one row so the transcript
	 * above it is easier to read. The row keeps focus and input routing, so the
	 * collapse key still routes here to expand. `t` stays inside the closure (live
	 * locale updates); the key display is static per session.
	 *
	 * With collapseKey "off" the router never toggles `collapsed`, but fall back to
	 * the cancel-only line rather than rendering a literal "Off to expand".
	 */
